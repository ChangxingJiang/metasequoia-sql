...
from metasequoia_sql_test.common.synatx_base import get_select_first_expression
...
class TestSimpleIdent(TestCase):
    ...
    def test_simple_ident_single(self):
        """测试单个标识符"""
        node = get_select_first_expression("SELECT column_name")
        self.assertTrue(isinstance(node, ast.Ident))
        self.assertEqual(node.value, "column_name")

    def test_simple_ident_2d(self):
        """测试两个点分隔的标识符"""
        node = get_select_first_expression("SELECT table_name.column_name")
    ...
    def test_simple_ident_3d(self):
        """测试三个点分隔的标识符"""
        node = get_select_first_expression("SELECT database_name.table_name.column_name")
