...
class FSMOperate(abc.ABC):
    ...
    def set_status(new_status: FSMStatus):
        ...
        return FSMOperateSetStatus(status=new_status)

    @staticmethod
    def add_cache(new_status: FSMStatus):
        """实例化 ADD_CACHE 类型的行为"""
        return FSMOperateAddCache(status=new_status)
    ...
    def add_and_handle_cache_to_wait(marks: int):
        ...
        return FSMOperateAddAndHandleCacheToWait(marks=marks, status=FSMStatus.WAIT)
...
class FSMOperateAddAndHandleCacheToWait(FSMOperate):
    ...
    def __init__(self, marks: int, status: FSMStatus):
        self.marks = marks
        self.status = status
