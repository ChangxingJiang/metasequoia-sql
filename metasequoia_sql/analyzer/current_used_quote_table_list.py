...
class CurrentTableAliasToQuoteHash(AnalyzerRecursionDictBase):
    """"获取当前层级（不递归分析子查询）中，表的表名到源表名的映射关系"""

    @classmethod
    def custom_handle_node(cls, node: SQLBase) -> Optional[Dict[QuoteTable, QuoteTable]]:
        """自定义的处理规则"""
        if (isinstance(node, SQLTableExpression)
                and node.alias is not None and isinstance(node.table, SQLTableNameExpression)):
            alias_name = QuoteTable(table_name=node.alias.name)
            table_name = QuoteTable(schema_name=node.table.schema, table_name=node.table.table)
...
class CurrentTableAliasToSubQueryHash(AnalyzerRecursionDictBase):
    ...
    @classmethod
    def custom_handle_node(cls, node: SQLBase) -> Optional[Dict[QuoteTable, SQLSubQueryExpression]]:
        """自定义的处理规则"""
        if (isinstance(node, SQLTableExpression)
                and node.alias is not None and isinstance(node.table, SQLSubQueryExpression)):
            alias_name = QuoteTable(table_name=node.alias.name)
            return {alias_name: node.table}
        if isinstance(node, SQLSubQueryExpression):
            return {}
        return None
